...
if 'game_started' not in st.session_state:
    ...
    with st.form("monthly_actions_form"):
        ...
        with c2:
            ...
            form_inputs['pay_kotak_sav'] = st.number_input("Pay Kotak from Savings (₹)", 0.0, step=100.0, max_value=st.session_state.accounts['Kotak']['current_debt'])
            form_inputs['pay_idfc_sav'] = st.number_input("Pay IDFC from Savings (₹)", 0.0, step=100.0, max_value=st.session_state.accounts['IDFC']['current_debt'])
            form_inputs['pay_super_sav'] = st.number_input("Pay Super from Savings (₹)", 0.0, step=100.0, max_value=st.session_state.accounts['Super']['current_debt'])
            
        st.divider()
        st.subheader("🔄 Balance Transfers (High Risk!)")
        
        c1, c2, c3 = st.columns(3)
        with c1:
            st.markdown("**Pay Kotak with...**")
            form_inputs['pay_kotak_from_idfc'] = st.number_input("...IDFC (₹)", 0.0, step=100.0, key="k_idfc", max_value=st.session_state.accounts['IDFC']['full_limit'] - st.session_state.accounts['IDFC']['current_debt'])
            form_inputs['pay_kotak_from_super'] = st.number_input("...Super (₹)", 0.0, step=100.0, key="k_sup", max_value=st.session_state.accounts['Super']['full_limit'] - st.session_state.accounts['Super']['current_debt'])
        with c2:
            st.markdown("**Pay IDFC with...**")
            form_inputs['pay_idfc_from_kotak'] = st.number_input("...Kotak (₹)", 0.0, step=100.0, key="i_kotak", max_value=st.session_state.accounts['Kotak']['full_limit'] - st.session_state.accounts['Kotak']['current_debt'])
            form_inputs['pay_idfc_from_super'] = st.number_input("...Super (₹)", 0.0, step=100.0, key="i_sup", max_value=st.session_state.accounts['Super']['full_limit'] - st.session_state.accounts['Super']['current_debt'])
        with c3:
            st.markdown("**Pay Super with...**")
            form_inputs['pay_super_from_kotak'] = st.number_input("...Kotak (₹)", 0.0, step=100.0, key="s_kotak", max_value=st.session_state.accounts['Kotak']['full_limit'] - st.session_state.accounts['Kotak']['current_debt'])
            form_inputs['pay_super_from_idfc'] = st.number_input("...IDFC (₹)", 0.0, step=100.0, key="s_idfc", max_value=st.session_state.accounts['IDFC']['full_limit'] - st.session_state.accounts['IDFC']['current_debt'])
            
        submitted_month = st.form_submit_button(f"End Month {st.session_state.month} & Process Actions")

    if submitted_month:
        process_month_end(form_inputs)
    ...
    if len(history_df) < 2:
        ...
        # Chart 1: Debts and Savings
        st.subheader("Total Debt & Savings Over Time")
        debt_df = history_df.melt(
            id_vars=['Month'],
            value_vars=['Savings_Balance', 'Kotak_Debt', 'IDFC_Debt', 'Super_Debt'],
            var_name='Account',
            value_name='Balance (₹)'
        )
        debt_chart = alt.Chart(debt_df).mark_line(point=True).encode(
        ...
        hp_df = history_df.melt(
            id_vars=['Month'],
            value_vars=['Savings_HP', 'Kotak_HP', 'IDFC_HP', 'Super_HP', 'Risk_Points'],
            var_name='Metric',
            value_name='Points'
        )
        ...
        # Chart 3: Monthly Flows
        st.subheader("Monthly Cash Flows")
        flow_df = history_df.melt(
            id_vars=['Month'],
            value_vars=['Total_Paid_From_Savings', 'Total_Balance_Transfers', 'Total_New_Expenses'],
            var_name='Flow',
            value_name='Amount (₹)'
        )
        # Filter out month 0 which has no flows
        flow_df = flow_df[flow_df['Month'] > 0]
        
        flow_chart = alt.Chart(flow_df).mark_bar().encode(
            x=alt.X('Month:O'), # Treat month as ordinal
            y=alt.Y('Amount (₹)'),
            color='Flow',
            xOffset='Flow',
